...
FileList *filelist_readbytes(char *project_name, FileList *filelist)
{
  ...
  if(filelist->file_removed == 0){
    ...
    filelist->file_size = 0;
    filelist->file_bytes = NULL;
  }
...
void filelist_write(char *project_name, FileList *filelist)
{
  ...
  // printf("DEBUG filelist_write:\n");
  // Add every file
  FileList *item = filelist;
  while (item != NULL)
  {
    // printf("ver: %d, file_removed: %d, file_path: %s, file_byte: %s\n",item->file_version, item->file_removed, item->file_path, item->file_bytes );
    if (item->file_removed == 0) {
      // Concatenate project_name to file_path
